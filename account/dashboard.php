...
$stmt=db()->prepare('SELECT id,request_number,destination,start_date,end_date,adults,children,budget_max,status,created_at FROM trip_requests WHERE user_id=? ORDER BY created_at DESC');$stmt->bind_param('i',$user['id']);$stmt->execute();$requests=$stmt->get_result();
