...
function provider_match_score(array $request, array $provider): float
{
    ...
    if ($max > 0 && ($providerMin > 0 || $providerMax > 0)) {
        ...
        if ($providerMin <= $max && $providerMax >= $min) $budget = 100.0;
        elseif ($providerMin <= ($max * 1.15) || $providerMax >= ($min * 0.85)) $budget = 70.0;
        else $budget = 30.0;
    }
...
function calculate_provider_namvoy_score(array $provider): float
{
    ...
    return round(($rating * 0.35) + ($response * 0.20) + ($cancellation * 0.20) + ($fulfillment * 0.15) + ($verified * 0.10), 2);
}
...
function load_provider_matching_profile(int $providerId): array
{
    ...
    $stmt->bind_param('i', $providerId); $stmt->execute(); $result=$stmt->get_result();
...
function enrich_provider_match(array $request, array $provider, ?array $profile = null): array
{
    ...
    $stmt->bind_param('is',$provider['id'],$destinationName); $stmt->execute(); $destinationRow=$stmt->get_result()->fetch_assoc();
    if($destinationRow) $destinationMatch=$destinationRow['expertise_level']==='expert'?100.0:90.0;
    $categoryMatch=0.0;
    if($travelType!=='' && in_array($travelType,$profile['categories'],true)) $categoryMatch=100.0;
    elseif($travelType==='' && count($profile['categories'])>0) $categoryMatch=75.0;
    $provider['destination_match']=$destinationMatch; $provider['category_match']=$categoryMatch;
...
function find_matching_providers(array $request,int $limit=20): array
{
    ...
    $stmt=db()->prepare("SELECT p.*,u.first_name,u.last_name FROM providers p INNER JOIN users u ON u.id=p.user_id WHERE p.verification_status IN ('verified','trusted','preferred') AND u.status='active' ORDER BY p.namvoy_score DESC,p.rating DESC,p.response_rate DESC LIMIT ?");
    $stmt->bind_param('i',$limit); $stmt->execute(); $result=$stmt->get_result(); $matches=[];
